...
export default function PerformaInvoicePreview({ challan, onBack, editable = false, onChange }) {
  ...
  function filterPerformaInvoiceFields(challan) {
  ...
  const filteredItems = Array.isArray(challan.items)
    ...
    : [];
  return {
    performaInvoiceNumber: challan.performaInvoiceNumber || "",
    date: challan.date || "",
    ...
    deliveryPhone: typeof challan.deliveryPhone === 'string' ? challan.deliveryPhone : '',
    ...
    PORef: typeof challan.PORef === 'string' ? challan.PORef : '',
  ...
  async function savePerformaInvoice(challan) {
    try {
      const filtered = filterPerformaInvoiceFields(challan);
      ...
      if (!filtered.performaInvoiceNumber || !filtered.date || !filtered.deliveryDate) {
        alert('Performa Invoice Number, Order Date, and Delivery Date are required.');
      ...
      let poNum = filtered.performaInvoiceNumber;
      const match = poNum.match(/^(?:PI[-\/]?)(\d{4})[-\/]?(\d{2}-\d{2})$/);
      if (match) {
        poNum = `PI/${match[1]}/${match[2]}`;
      } else {
        const numMatch = poNum.match(/(\d{4})/);
        const yearMatch = poNum.match(/(\d{2}-\d{2})/);
        if (numMatch && yearMatch) {
          poNum = `PI/${numMatch[1]}/${yearMatch[1]}`;
        }
      }
      const method = challan.isEditing ? 'PUT' : 'POST';
      const url = challan.isEditing
        ? `/api/performa-invoices/${encodeURIComponent(poNum)}`
        : '/api/performa-invoices';
      ...
      if (!response.ok) {
        ...
        alert(errorMsg || 'Failed to save performa Invoice');
        throw new Error(errorMsg || 'Failed to save performa Invoice');
      }
      return await response.json();
    } catch (error) {
      alert(error.message || 'Error saving performa Invoice');
      throw error;
    }
  }
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
        ...
        pdf.save(`PerformaInvoice_${challan.performaInvoiceNumber || ''}.pdf`);
    } catch (error) {
  ...
  <div style={{ position: 'relative', zIndex: 1, marginBottom: 8, display: 'flex', justifyContent: 'center' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'flex-end', marginBottom: 2, width: '100%' }}>
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            ...
            value={challan.performaInvoiceNumber || ""}
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 32, marginBottom: 12 }}>
        ...
        <div style={{ flex: 1 }}>
            ...
            {editable ? (
              ...
              >
                {challan.deliveryAddress_address || ''}
              </div>
            )}
            </div>
            <div style={{ marginBottom: 8 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: 24, marginTop: 32 }}>
        {editable && (
            ...
            onClick={async () => {
                try {
                    await savePerformaInvoice(challan);
